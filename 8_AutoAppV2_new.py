...
def edit_info_plist(selected_folder: str, bundle_id_val: str, app_name_val: str):
    """
    Info.plist 파일을 수정합니다.
    Google AdMob을 제외한 Unity Ads, ironSource, Pangle, AppLovin 관련 모든 항목을 제거합니다.
    """
    print("\n--- Step 8: Info.plist 파일 수정 및 Ad Network ID 제거 ---")
    project_folder = os.path.join(selected_folder, bundle_id_val)
    ...
    # ----------------------------------------------------------------------
    # 1. Google AdMob에 필요한 SKAdNetworkIdentifier만 유지
    # ----------------------------------------------------------------------
    # Google AdMob에서 공식적으로 요구하는 필수 ID들만 유지합니다
    admob_required_ids = [
        # Google AdMob 필수 ID (Google 공식 문서 기준)
        {"SKAdNetworkIdentifier": "cstr6suwn9.skadnetwork"},  # Google
        {"SKAdNetworkIdentifier": "4fzdc2evr5.skadnetwork"},  # Google  
        {"SKAdNetworkIdentifier": "4pfyvq9l8r.skadnetwork"},  # Google
        {"SKAdNetworkIdentifier": "2fnua5tdw4.skadnetwork"},  # Google
        {"SKAdNetworkIdentifier": "ydx93a7ass.skadnetwork"},  # Google
        {"SKAdNetworkIdentifier": "5a6flpkh64.skadnetwork"},  # Google
        {"SKAdNetworkIdentifier": "p78axxw29g.skadnetwork"},  # Google
        {"SKAdNetworkIdentifier": "v72qych5uu.skadnetwork"},  # Google
        {"SKAdNetworkIdentifier": "mlmmfzh3r3.skadnetwork"},  # Google
        {"SKAdNetworkIdentifier": "c6k4g5qg8m.skadnetwork"},  # Google
        {"SKAdNetworkIdentifier": "wg4vff78zm.skadnetwork"},  # Google
        {"SKAdNetworkIdentifier": "g28c52eehv.skadnetwork"},  # Google
        {"SKAdNetworkIdentifier": "uw77j35x4d.skadnetwork"},  # Google
        {"SKAdNetworkIdentifier": "hs6bdukanm.skadnetwork"},  # Google
        
        # 주요 DSP/Exchange (AdMob 파트너)
        {"SKAdNetworkIdentifier": "3rd42ekr43.skadnetwork"},  # Aarki
        {"SKAdNetworkIdentifier": "2u9pt9hc89.skadnetwork"},  # Adjust
        {"SKAdNetworkIdentifier": "7ug5zh24hu.skadnetwork"},  # AdColony
        {"SKAdNetworkIdentifier": "klf5c3l5u5.skadnetwork"},  # AdGate Media
        {"SKAdNetworkIdentifier": "t38b2kh725.skadnetwork"},  # Amazon
        {"SKAdNetworkIdentifier": "9t245vhmpl.skadnetwork"},  # Appier
        {"SKAdNetworkIdentifier": "kbd757ywx3.skadnetwork"},  # Appnext
        {"SKAdNetworkIdentifier": "n38lu8286q.skadnetwork"},  # Bytedance (non-Pangle)
        {"SKAdNetworkIdentifier": "v9wttpbfk9.skadnetwork"},  # Chartboost
        {"SKAdNetworkIdentifier": "f73kdq92p3.skadnetwork"},  # Criteo
        {"SKAdNetworkIdentifier": "44jx6755aq.skadnetwork"},  # Epsilon
        {"SKAdNetworkIdentifier": "4468km3ulz.skadnetwork"},  # Hybrid
        {"SKAdNetworkIdentifier": "e5fvkxwrpn.skadnetwork"},  # InMobi
        {"SKAdNetworkIdentifier": "8s468mfl3y.skadnetwork"},  # Liftoff
        {"SKAdNetworkIdentifier": "av6w8kgt66.skadnetwork"},  # Moloco
        {"SKAdNetworkIdentifier": "22mmun2rn5.skadnetwork"},  # MoPub (Twitter)
        {"SKAdNetworkIdentifier": "lr83yxwka7.skadnetwork"},  # Persona.ly
        {"SKAdNetworkIdentifier": "24t9a8vw3c.skadnetwork"},  # PubNative
        {"SKAdNetworkIdentifier": "578prtvx9j.skadnetwork"},  # Smaato
        {"SKAdNetworkIdentifier": "ecpz2srf59.skadnetwork"},  # Sonobi
        {"SKAdNetworkIdentifier": "v79kvwwj4g.skadnetwork"},  # Tapjoy
        {"SKAdNetworkIdentifier": "3sh42y64q3.skadnetwork"},  # Verizon
        {"SKAdNetworkIdentifier": "5lm9lj6jb7.skadnetwork"},  # Vungle (Liftoff)
        {"SKAdNetworkIdentifier": "32z4fx6l9h.skadnetwork"},  # Adelphic
    ]
    ...
    try:
        ...
        plist_data.update({
            ...
            # Google AdMob에 필요한 ID만 포함
            "SKAdNetworkItems": admob_required_ids,
        })

        # ----------------------------------------------------------------------
        # 3. 불필요한 광고 SDK 관련 키 제거 (있을 경우)
        # ----------------------------------------------------------------------
        keys_to_remove = [
            "UnityAdsAppStoreId",           # Unity Ads
            "IronSourceAppKey",             # ironSource  
            "ISAdMobAppKey",                # ironSource AdMob adapter
            "AppLovinSdkKey",               # AppLovin
            "ALSdkKey",                     # AppLovin alternative key
            "PangleAppID",                  # Pangle
            "BytedanceAppID",               # Pangle/TikTok
            "FacebookAppID",                # Facebook (사용하지 않는 경우)
            "FacebookDisplayName",          # Facebook (사용하지 않는 경우)
            "VungleAppID",                  # Vungle (사용하지 않는 경우)
            "ChartboostAppID",              # Chartboost (사용하지 않는 경우)
            "ChartboostAppSignature",       # Chartboost (사용하지 않는 경우)
        ]
        ...
        # 수정된 plist 데이터를 파일에 다시 저장
        with open(info_plist_path, "wb") as f:
            plistlib.dump(plist_data, f)
            
        print("✅ Info.plist 파일 수정 완료:")
        print(f"  - Bundle ID: {bundle_id_val}")
        print(f"  - App Name: {app_name_val}")
        print(f"  - SKAdNetworkItems: {len(admob_required_ids)}개 (Google AdMob용만 유지)")
        print("  - Unity Ads, ironSource, Pangle, AppLovin 관련 항목 모두 제거됨")

    except Exception as e:
        print(f"❌ Info.plist 수정 중 예외 발생: {e}")
